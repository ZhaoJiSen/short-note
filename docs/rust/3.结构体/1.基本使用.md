...
结构体更新指的是：复用旧实例的大部分字段，但替换少量字段后，来创建一个新的结构体实例。语法为 `..`，表示未显式设置的字段取自给定实例对应字段的值
...
> [!IMPORTANT]
> 使用 `..user1` 时，`user1` 中的非 `Copy` 字段（如 `String`）会被移动到新实例，之后 `user1.username` 无法再用；`bool` 和 `u64` 这类实现了 `Copy` 的字段会被按值复制
> 如需保留原实例的所有字段，可在需要的字段上调用 `clone`
