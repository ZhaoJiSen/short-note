...
- serde：默认 `std`；可选 `derive`（派生宏）、`alloc`（无 std 环境）、`rc`（支持 Rc/Arc）、`unstable`（夜间特性）
- serde_json：默认 `std`；可选 `alloc`（无 std 环境）、`preserve_order`（保留 map 顺序）、`raw_value`（延迟解析 Value）、`float_roundtrip`（浮点精确往返）、`arbitrary_precision`（高精度数字）、`unbounded_depth`（禁用嵌套深度限制）

## 高级用法

::: details 常见高级属性
- `rename` / `rename_all`：字段或枚举批量改名
- `alias`：兼容旧字段名（只在反序列化生效）
- `skip` / `skip_serializing` / `skip_deserializing`：按需忽略字段
- `default` / `default = "fn"`：缺省字段兜底
- `with = "module"` / `serialize_with` / `deserialize_with`：自定义序列化逻辑
- `tag` / `content` / `untagged`：枚举标记策略
- `flatten`：展开嵌套结构，或接收未知字段
- `transparent`：newtype 透明包装
- `deny_unknown_fields`：拒绝未知字段，防止输入里有脏数据
:::
