...
[+credentials]:
  ...
  1. `omit`：不携带凭证
  2. `same-origin`：仅在同域请求中携带凭证
  3. `include`：无论是否同域都携带凭证
...
| mode | 请求模式，可选值有 cors、no-cors、same-origin，默认为 cors |
| credentials | 是否携带凭证，如 cookies，可选值有 omit、same-origin、include，默认为 same-origin |
