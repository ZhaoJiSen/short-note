...
- 整个 LLM 是一串堆叠的 Transformer 块，每个块 = **自注意力层** + **前馈神经网络层**，首尾相接
- **自注意力**：当前词元去关注上下文里的所有词，按相关性分配 **注意力权重**，把上下文信息整合进自己的表示 —— "看别人"
- **多头注意力**：多个头并行从不同视角（指代 / 句法 / 逻辑……）关注，再合并成一个语义极丰富的向量 —— 这正是"只取最后一个词元就能预测"的底气
- **前馈网络**：对每个词元 **单独** 做"升维 → GELU 激活 → 降维"的变换，让它自己提炼、深化表示 —— "想自己"
- 一个是横向整合上下文，一个是纵向深化单词元，两者配合，块与块堆叠，模型才越来越能理解语言
