...
[+CreateContentBrowserClient]: 该方法是通过 `ElectronMainDelegate` 复写的 Chrome 的 `ContentBrowserClient` 方法
...
2. 创建 ElectronBrowserClient 对象
   ...
   通过 `ElectronMainDelegate` 注入代码后。通过 `CreateContentBrowserClient`[+CreateContentBrowserClient] 方法，创建 `ElectronBrowserClient` 对象

   在这个对象中定义了在这个对象中定义了 Electron 对 Chromium Browser Process 的定制逻辑
...
3. 创建 BrowserMainParts
   ...
   在 `BrowserMainLoop`启动过程中就会调用 `ElectronBrowserClient` 的 `CreateBrowserMainParts` 方法，该方法负责创建并返回 `ElectronBrowserMainParts` 对象[+ElectronBrowserMainParts]，用于管理主进程的启动事件
