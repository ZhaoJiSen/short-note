...
2. 对外暴露 createApp
   ...
   >
   > 而在 SSR 应用中，应用[+应用]通常只在服务器启动时初始化一次，如果直接复用同一个实例去处理所有请求，请求之间就会共享状态进而导致 "跨请求状态污染" 的问题。所以为了避免这个问题，我们将 `createApp` 写成工厂函数，每个请求调用一次，返回一组全新的 App/Router/Store 实例，这样状态就被隔离了
...
4. Hydration
   ...
   app.use('/client', express.static(resolve(__dirname, '../dist/client'))); // [!code focus]
   ...
   app.get('/', async (req, res) => {
     ...
     res.send(
       [
         ...
         '<body>',
         '<div id="app">' + appString + '</div>',
         '<script src="/client/client_bundle.js"></script>', // [!code focus]
         '</body>',
       ].join('\n'),
     );
   });
...
5. 路由的实现
   ...
   const { resolve } = require('path');
   const { createMemoryHistory } = require('vue-router'); // [!code focus]
   const { renderToString } = require('@vue/server-renderer');
   const { createRouterInstance } = require('../frontend/router'); // [!code focus]
   ...
   app.get('/*', async (req, res) => {
     ...
     // 服务端没有浏览器环境（location/history API 不存在），必须用内存路由避免依赖 DOM  // [!code focus]
     let router = createRouterInstance(createMemoryHistory());  // [!code focus]

     vueApp.use(router);

     // 先把服务端路由切到当前请求的 URL，确保渲染的是正确页面  // [!code focus]
     // 如果不在服务端显式地将路由切换到当前请求路径，SSR 将始终渲染默认路由对应的页面，导致返回的 HTML 与实际 URL 不匹配  // [!code focus]
     await router.push(req.url || '/');  // [!code focus]
     // 等待路由就绪（包含异步路由组件/路由守卫），避免 SSR 渲染出错或内容不完整  // [!code focus]
     await router.isReady();  // [!code focus]
...
6. Store 的实现
   ...
   > [!IMPORTANT]
   > 为了避免首屏 hydration mismatch，需要在服务端渲染后导出初始 state，并在==客户端挂载前==恢复到 Pinia 中，使两端的首屏状态一致
   ...
   import createApp from './main';
   import { createWebHistory } from 'vue-router';
   import { createPiniaInstance } from './store';  // [!code focus]
   import { createRouterInstance } from './router';

   const { app } = createApp();
   const pinia = createPiniaInstance();  // [!code focus]
   const router = createRouterInstance(createWebHistory());  

   app.use(router);
   app.use(pinia);  // [!code focus]

   // 服务端注入的初始 state 会挂在 window 上，客户端先恢复再挂载 // [!code focus]
   if (window.__INITIAL_STATE__) {  // [!code focus]
     pinia.state.value = window.__INITIAL_STATE__; // [!code focus]
   } // [!code focus]
   ...
   const { renderToString } = require('@vue/server-renderer');
   const { createPiniaInstance } = require('../frontend/store');  // [!code focus]
   ...
   app.get('/*', async (req, res) => {
     ...
     let pinia = createPiniaInstance();  // [!code focus]
     ...
     // 将服务端初始 state 序列化注入 HTML，供客户端恢复  // [!code focus]
     // replace 防止 </script> 提前闭合（XSS / HTML 注入）  // [!code focus]
     let state = JSON.stringify(pinia.state.value).replace(/</g, '\\u003c'); // [!code focus]
     ...
     res.send(
       [
         ...
         '<body>',
         '<div id="app">' + appString + '</div>',
         '<script>window.__INITIAL_STATE__= '+ state + '</script>', // [!code focus]
         '<script src="/client/client_bundle.js"></script>',
         '</body>',
       ].join('\n'),
     );
   ...
   > [!NOTE]
   > `window.__INITIAL_STATE__ ` 并不是全局或持久状态，它只存在于当前请求返回的 HTML 中，每一次页面级请求都会重新执行 SSR 流程，并生成一份新的初始状态快照，因此不存在因用户 "非首次访问" 而导致状态未注入或被复用的问题
