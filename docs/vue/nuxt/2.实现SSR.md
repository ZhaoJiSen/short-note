...
5. 路由的实现
   ...
   在真实场景下，一个 SSR 应用往往需要根据不同的 URL 返回不同的页面内容，这也就引入的路由系统

   在 SSR 场景中，路由需要同时参与 ==服务端渲染阶段[+服务端渲染阶段]== 与 ==客户端激活阶段[+客户端激活阶段]==。路由不再只是前端行为，而是服务端渲染流程的一部分

   > [!IMPORTANT]
   > 在 vue-router 中，路由的 history 实现是与运行环境强相关的，而 SSR 应用同时运行在 服务端（Node） 和 客户端（浏览器） 两种环境中，因此不能在路由模块中直接写死某一种 history 类型。在服务端中需要使用 `createMemoryHistory`，在客户端中可以使用 `createWebHashHistory` 或 `createWebHistory`
