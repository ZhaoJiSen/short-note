...
1. SSR 首次请求
   ...
   :::details 具体流程
   1. 首先执行 `server/plugins`（仅 Nitro 启动时一次），用于注册 Nitro hooks 与运行时配置  
   ...
   5. 接着执行路由 `validate` 校验，确保参数与访问合法  
   6. 然后执行路由中间件（`app/middleware`），完成准入与跳转逻辑  
   7. 之后运行 **服务端 `setup`**，并同步触发 `onServerPrefetch`、`useFetch/useAsyncData` 获取首屏数据  
   8. 渲染完成触发 `app:rendered`，表示 HTML 已准备好  
   9. 接着触发 `render:html`，进行最终 HTML 处理与注入  
   10. 最后进入 `render:response / beforeResponse / afterResponse` 并发送响应，结束本次请求
   :::
