...
> `useFetch / useAsyncData` 用于 **SSR 友好的首屏数据获取**，`$fetch` 只用于 **客户端事件**
> 在 `setup` 中直接使用 `$fetch` 会导致 SSR + CSR 双次请求

:::details 最佳实践
1. 首屏数据使用 `await useFetch` 或 `await useAsyncData`，不要用 `$fetch`
...
- **结论**：在 `setup` 里必须用 `useFetch/useAsyncData`
