...
除了 `app` 与 `public`，`shared / server / layers` 也是 Nuxt 项目里经常会用到的目录。

#### `shared`：跨端共享代码

`shared` 适合放前后端都能复用的 **纯 TypeScript 代码**，比如类型定义、枚举常量、校验 schema、纯函数工具。
...
`layers` 用于复用一套 Nuxt 能力（页面、布局、组件、插件、配置等），适合多项目共享基础能力。
...
> [!TIP]
> 想看 `shared` 与 `layers` 的合并优先级、覆盖策略与常见坑，可以继续阅读原理篇：[`shared 与 layers 进阶`](/vue/rxe54a2m/)


## 应用入口

[+NuxtPage]: 相当于 `vue-router` 的 `RouterView`
[+NuxtLayout]: `NuxtLayout` 会根据页面的 `definePageMeta({ layout })` 切换布局，默认使用 `app/layouts/default.vue`
