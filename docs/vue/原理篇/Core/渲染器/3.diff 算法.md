...
8. 使用最长递增子序列减少移动
   ...
   如果确定需要移动，并不代表所有节点都要移动。`source` 中最长递增子序列对应的那些节点，在新旧列表中的相对顺序没有变化，可以保持不动
...
然后就是双端 Diff，双端 Diff 说的是：先从两端开始比较，它每次只比较四种情况
...
最后就是快速 Diff，快速 Diff 说的是：先预处理两端相同的节点，剩下的就是中间区间了
