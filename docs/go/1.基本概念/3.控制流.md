...
1. 经典三段式：`for init; cond; post {}`，最接近 C/Java 的 `for`。
2. 条件式：`for cond {}`，等价于其他语言的 `while`。
3. 无限循环：`for {}`，常搭配 `break`/`return` 退出。

::: go-repl title="for 的三种形态与 break/continue"
```go
package main
